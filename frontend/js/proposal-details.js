...
function voteLabel(vote){ return vote === 'yes' ? 'может' : vote === 'no' ? 'не может' : vote === 'maybe' ? 'под вопросом' : 'ждём ответ'; }
...
function renderProposalCard(item, showCreator = true){
  ...
  const maybeClass = item.current_user_vote === 'maybe' ? 'is-active' : '';
  ...
  return `<div class="proposal-compact-card" data-proposal-id="${item.id}">
    ...
    <div class="proposal-actions-row">
      ...
      <button class="proposal-vote-btn proposal-vote-btn--maybe ${maybeClass}" type="button" data-vote="maybe">Под вопросом</button>
